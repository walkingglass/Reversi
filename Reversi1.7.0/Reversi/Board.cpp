...
void Board::ShowBoard(short wave)
{
    ...
    for (uint8_t y = 0; y < BOARD_SIZE; y++)
    {
        ...
        for (uint8_t x = 0; x < BOARD_SIZE; x++)
        {
            ...
            { 
                if(set_board.hint_chess_open==true)
    ...
    printf("\33[10;%dH╔═════╗", point);
    printf("\33[11;%dH║ 悔棋║", point);
    printf("\33[12;%dH", point);
    printf("╚═════╝");
...
void Board::LoadFromFile(const char* folderPath, uint8_t filenum)
{
    ...
    if (findHandle == -1) {
        std::cerr << "目錄中未找到文件: " << searchPath << std::endl;
        return;
    }
        ...
        if (!(fileData.attrib & _A_SUBDIR)) // 如果不是子目錄，處理文件
        {
            ...
            if (currentFileNum < 250)
            {
                ...
                if (filenum >= 0 && currentFileNum == filenum)
                {
                    ...
                    {
                        std::cerr << "無法開啟檔案進行讀取: " << filePath << std::endl;
                    }
                    break;
                }
                currentFileNum++;
            ...
            {
                std::cerr << "找到太多文件.達到最大限制250." << std::endl;
                break;
            }
        }
...
void Board::AICount(uint8_t who, short wave)
{
    max_point = -32768;
    max_x = 0;
    max_y = 0;
    ...
    for (uint8_t y = 0; y < BOARD_SIZE; y++)
        for (uint8_t x = 0; x < BOARD_SIZE; x++)
            ...
            {
                for (uint8_t i = 0; i < 8; i++)
                    board[x][y].point += set_board.normal_point[who] *RandomNumber(set_board.random[who]) * ExtractDigits("HIGH", board[x][y].judge_direction[i]);

                if (x == 0 || x == BOARD_SIZE - 1)
                    board[x][y].point += set_board.bonus_point[who] * RandomNumber(set_board.random[who]);
                if (y == 0 || y == BOARD_SIZE - 1)
                    board[x][y].point += set_board.bonus_point[who] * RandomNumber(set_board.random[who]);

                if (x == 1 || x == BOARD_SIZE - 2)
                    board[x][y].point += set_board.penalty_point[who] * RandomNumber(set_board.random[who]);
                ...
                if (x >= center)
                    board[x][y].point += (x - center) * set_board.distance_point[who] * set_board.distance_point_variation[who];
                else if (x <= center)
                    board[x][y].point += (center - x) * set_board.distance_point[who] * set_board.distance_point_variation[who];

                if (y >= center)
                    board[x][y].point += (y - center) * set_board.distance_point[who] * set_board.distance_point_variation[who];
                else if (y <= center)
                    board[x][y].point += (center - y) * set_board.distance_point[who] * set_board.distance_point_variation[who];
...
void Board::regret_chess(short &wave)
{
    if (wave > 1)
    {
        for (uint8_t y = 0; y < BOARD_SIZE; y++)
            for (uint8_t x = 0; x < BOARD_SIZE; x++)
                board[x][y] = board_history[x][y][wave - 2];
        wave-=1;
    }
}
