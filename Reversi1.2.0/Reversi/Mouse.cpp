...
void Mouse::setCursorPosition(short int x, short int y)
{
    COORD coord;
    coord.X = x;
    coord.Y = y;
...
void Mouse::updateMousePosition(short int x, short int y)
{
    mouseX = x;
    mouseY = y;
}
...
int Mouse::MouseTracking(const char* show_string, short int show_x, short int show_y)
{
        ...
        if (ReadConsoleInput(hIn, irInBuf, 128, &cNumRead))//讀取控制台輸入事件
        {
            for (DWORD i = 0; i < cNumRead; i++)//遍歷讀取到的事件記錄
            {
                if (irInBuf[i].EventType == MOUSE_EVENT)//判斷目前事件是否為滑鼠事件
                {
                    ...
                    {
                        //為了符合棋子格子的運算
                        short int mouseX = mouseEvent.dwMousePosition.X / 2 - 1;
                        short int mouseY = mouseEvent.dwMousePosition.Y - 1;
                    ...
                    {
                        //滑鼠位置(為了符合格子的運算)
                        short int mouseX = mouseEvent.dwMousePosition.X;
                        short int mouseY = mouseEvent.dwMousePosition.Y;
        ...
        if (ReadConsoleInput(hIn, irInBuf, 128, &cNumRead))//讀取控制台輸入事件
        {
            for (DWORD i = 0; i < cNumRead; i++)//遍歷讀取到的事件記錄
            {
                if (irInBuf[i].EventType == MOUSE_EVENT)//判斷目前事件是否為滑鼠事件
                {
                    ...
                    {
                        // 計算控制台寬度和高度（像素）
                        short int consoleWidth = consoleRect.right - consoleRect.left;
                        short int consoleHeight = consoleRect.bottom - consoleRect.top;
    ...
    while (true)//一直在讀取
    {
        if (ReadConsoleInput(hIn, irInBuf, 128, &cNumRead))//讀取控制台輸入事件
        {
            for (DWORD i = 0; i < cNumRead; i++)//遍歷讀取到的事件記錄
            {
                ...
                {
                    MOUSE_EVENT_RECORD mouseEvent = irInBuf[i].Event.MouseEvent;//滑鼠事件
                    short int mouseX = mouseEvent.dwMousePosition.X;//滑鼠位置
                    short int mouseY = mouseEvent.dwMousePosition.Y;
...
void Mouse::ScrollConsole(short int lines)
{
    // 獲取標準輸出控制台的處理
