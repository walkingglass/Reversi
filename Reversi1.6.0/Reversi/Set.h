...
public:
	...
	static short normal_point[2];    //連著的不同棋加分						 (預設100)
	static short bonus_point[2];     //最邊線加分							 (預設1000)
	static short penalty_point[2];   //[最邊線-1]扣分						 (預設-1800)
	static short distance_point[2];//中心擴散加分							 (預設10)
	static short distance_point_variation[2];//中心擴散加分倍率				 (預設1)
	static double random[2];         //AI人性隨機數							 (預設0.9)(0~1)
	static short delar_time;      //電腦下棋速度(延遲)(越低增加電腦負擔)	 (可設置0)
	...
	static short Number_of_executions;//AI調適次數                           (預設100)
